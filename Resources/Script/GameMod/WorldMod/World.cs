...
namespace WorldMode
{
    public class World : IMode
    {
        ...
        public World(GameObject player, MonoBehaviour monoBehaviour)
        {
            ...
            _levelObject = canvas.transform.GetChild(2).GetChild(0).gameObject.GetComponent<TMP_Text>();
        ...
        private void On1LevelStart()
        {
            ...
            LevelGenerator levelGenerator = new LevelGenerator("Level1", _agentList, _player, _tileController);
            ...
            _patchHolders[1].addEnemy(_slimeMagPref, 170);
        ...
        private void On2LevelStart()
        {
            ...
            List<List<Tile>> tileMap = new List<List<Tile>>();
            _tileController = new TileController(tileMap);
            LevelGenerator levelGenerator = new LevelGenerator("Level2", _agentList, _player, _tileController);
            levelObject = levelGenerator.GetLevelObject();
            _playerAction = levelGenerator.GetPlayerAction();
        ...
        private void On3LevelStart()
        {
            ...
            List<List<Tile>> tileMap = new List<List<Tile>>();
            _tileController = new TileController(tileMap);
            LevelGenerator levelGenerator = new LevelGenerator("Level3", _agentList, _player, _tileController);
            levelObject = levelGenerator.GetLevelObject();
            _playerAction = levelGenerator.GetPlayerAction();
        ...
        private void onLevelEnd(MonoBehaviour monoBehaviour)
        {
            ...
            //Åñëè óðîâåíü çàêîí÷åíóñïåøíî äîæåí áûòü æèâ êàê ìèíèìóì îäèí àãåíò. Íóæíî áóäåò èçìåíèòü âî èçáåæàíèå èñêëþ÷åíèé
            for (int i = 0; i < _agentList.Count; i++)
            {
                Object.Destroy(_agentList[i].GetAgentObject());
            }

    ...
    public List<EnemyAction> SpawnCheker(float timer,TileController tileController)
    {
        ...
        for(int i=0;i<enemies.Count;)
        {
            if (enemies[i].ReadyToSpawn(timer))
            {
                ...
                {
                    case "Spider":
                        GeneralLiveForm Spider = new GeneralLiveForm(5, 4, 2, 6, false, 0.003f, (tileController.SpawnOnTile(enemies[i].GetEnemy(), (int)spawner.x, (int)spawner.z)));
                    ...
                    case "SpiderMag":
                        GeneralLiveForm SpiderMag = new GeneralLiveForm(5, 0, 4, 7, true, 0.003f, (tileController.SpawnOnTile(enemies[i].GetEnemy(), (int)spawner.x, (int)spawner.z)));
                        radius = new List<Point>() { new Point(0, 1) };
                    ...
                    case "SpiderSpeed":
                        GeneralLiveForm SpiderSpeed = new GeneralLiveForm(5, 1, 1, 4, false, 0.005f, (tileController.SpawnOnTile(enemies[i].GetEnemy(), (int)spawner.x, (int)spawner.z)));
                        radius = new List<Point>() { new Point(0, 1) };

...
namespace WorldMode
{
    namespace Player
    {
        public class PlayerAction : BasicAction
        {
            ...
            public void AgentDetach(MonoBehaviour monoBehaviour)
            {
                if (_actionLiveForm.GetGameObject().transform.childCount > 1)
                {
                    StopCourutine(monoBehaviour);
                    
                    _actionLiveForm.GetGameObject().transform.position = _endMove;
                    _actionLiveForm.GetGameObject().transform.eulerAngles= _endRotate;
                    _startRotation = _player.transform.eulerAngles=_endRotate;
                    _startMove = _player.transform.position=_endMove;
                    _agent.GetAgentAction().SetStartPositin(_startRotation, _startMove);
                    _inAction = false;
